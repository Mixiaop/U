...
namespace U.Segmentation.Jieba.PosSeg
{
    public class Viterbi
    {
        ...
        private static JiebaSettings _settings = U.UPrimeEngine.Instance.Resolve<JiebaSettings>();
        private Viterbi()
        {
            LoadModel();
        }
        ...
        {
            var startJson = File.ReadAllText( _settings.PosProbStartFileName.GetResourcePath());
            _startProbs = JsonConvert.DeserializeObject<IDictionary<string, double>>(startJson);

            var transJson = File.ReadAllText(_settings.PosProbTransFileName.GetResourcePath());
            _transProbs = JsonConvert.DeserializeObject<IDictionary<string, IDictionary<string, double>>>(transJson);

            var emitJson = File.ReadAllText(_settings.ProbEmitFileName.GetResourcePath());
            _emitProbs = JsonConvert.DeserializeObject<IDictionary<string, IDictionary<char, double>>>(emitJson);

            var tabJson = File.ReadAllText(_settings.CharStateTabFileName.GetResourcePath());
