...
{
    public class IIS6UtilService : BaseService, IIISUtilService
    {
        public int CreateSite(string siteName, int httpPort, int httpsPort, string sslHash, string physicalPath)
        {
            ...
            try
            {
                ...
                {
                    if (!site.SchemaClassName.Equals("IIsWebServer", StringComparison.CurrentCultureIgnoreCase))		//IIsWebServer代表站点
                        continue;
                    ...
                    if (!name.IsNullOrEmpty() && name.Equals(siteName, StringComparison.CurrentCultureIgnoreCase))
                        exists = true;
                }
        ...
        public int RemoveSite(string siteName)
        {
            ...
            try
            {
                ...
                foreach (DirectoryEntry site in siteEntry.Children)
                {
                    if (!site.SchemaClassName.Equals("IIsWebServer", StringComparison.CurrentCultureIgnoreCase))
                        continue;
        ...
        public int CreateAppPool(string poolName)
        {
            ...
            try
            {
                ...
                foreach (DirectoryEntry pool in poolEntry.Children)
                {
                    ...
                    if (pool.Name.Equals(poolName, StringComparison.CurrentCultureIgnoreCase) && pool.SchemaClassName.Equals("IIsApplicationPool", StringComparison.CurrentCultureIgnoreCase))
                    {
                        exists = true;
                        break;
                    }
            ...
            try
            {
                ...
                if (targetSite != null)
                {
                    ...
                    foreach (DirectoryEntry dir in rootEntry.Children)
                    {
                        ...
                        if (!dir.SchemaClassName.Equals("IIsWebVirtualDir", StringComparison.CurrentCultureIgnoreCase))
                            continue;
