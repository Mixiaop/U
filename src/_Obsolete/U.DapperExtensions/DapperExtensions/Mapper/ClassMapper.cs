...
namespace U.DapperExtensions.Mapper
{
    ...
    public class ClassMapper<T> : IClassMapper<T> where T : class
    {
        ...
        public PropertyMap MapLeftJoin(Expression<Func<T, object>> leftJoinProperty, string leftJoinForeignColumnName, IClassMapper leftJoinMapper, string leftJoinPrimaryColumnName)
        {
            PropertyInfo propertyInfo = ReflectionHelper.GetProperty(leftJoinProperty) as PropertyInfo;
            PropertyMap result = new PropertyMap(propertyInfo, leftJoinMapper, leftJoinPrimaryColumnName, leftJoinForeignColumnName);
        ...
        public void SetMultiQueryIgnoreColumns(string ignoreColumns) {
            MultiQueryIgnoreColumns = ignoreColumns;
        }

        public void SetMultiQuerySplitOn(string splitOn) {
