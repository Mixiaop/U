    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public virtual string BuildSelectColumnsByLeftJoin(IClassMapper classMap)
        {
            ...
            if (classMap.HaveLeftJoinProperties)
            {
                ...
                {
                    if (p.LeftJoinDisplayColumns.IsNullOrEmpty())
                    {
                        columnsSql += "," + BuildSelectColumns(p.LeftJoinMapper);
                    }
                    else {
                        string displayColumns = string.Empty;
                        string[] columns = p.LeftJoinDisplayColumns.Split(",");
                        if (columns != null && columns.Length > 0) {
                            var tableName = GetTableName(p.LeftJoinMapper);
                            foreach (var column in columns) {
                                displayColumns += tableName + "." + column + ",";
                            }

                            displayColumns = displayColumns.TrimEnd(",");
                        }
                        if (displayColumns.IsNotNullOrEmpty())
                            columnsSql += "," + displayColumns;
                    }
