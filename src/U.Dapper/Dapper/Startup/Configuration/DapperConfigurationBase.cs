...
{
    public class DapperConfigurationBase : IDapperConfiguration
    {
        private readonly ConcurrentDictionary<Type, IClassMapper> _classMaps = new ConcurrentDictionary<Type, IClassMapper>();

        public DapperConfigurationBase()
            : this(typeof(AutoClassMapper<>), new List<Assembly>(), new SqlServerDialect())
        {
        }


        public DapperConfigurationBase(Type defaultMapper, IList<Assembly> mappingAssemblies, ISqlDialect sqlDialect)
        {
        ...
        public virtual bool OpenedReadAndWrite { get; set; }

        public virtual string SqlConnectionString { get; set; }

        public virtual string ReadSqlConnectionString { get; set; }
    }
}
