...
namespace U.Logging
{
    public class ULog4netFileAppender : RollingFileAppender
    {
        /// <summary>
            ...
            lock (this)
            {
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++)
                {
                    ...
                    catch
                    {
                        ...
                        LogLog.Error(string.Format("ULog4netFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
