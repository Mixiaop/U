...
namespace U.Domain.Uow
{
    ...
    public class CallContextCurrentUnitOfWorkProvider : ICurrentUnitOfWorkProvider, ITransientDependency
    {
        ...
        private const string ContextKey = "U.UnitOfWork.Current";
        ...
        private static IUnitOfWork GetCurrentUow(ILogger logger)
        {
            ...
            {
                logger.Warning("There is a unitOfWorkKey in CallContext but the UOW was disposed!");
        ...
        private static void SetCurrentUow(IUnitOfWork value, ILogger logger)
        {
            ...
            if (unitOfWorkKey != null)
            {
                ...
                if (UnitOfWorkDictionary.TryGetValue(unitOfWorkKey, out outer))
                {
                    if (outer == value)
                    {
                        logger.Warning("Setting the same UOW to the CallContext, no need to set again!");
                        return;
                    }
            ...
            {
                throw new UException("Can not set unit of work! UnitOfWorkDictionary.TryAdd returns false!");
            }
        ...
        private static void ExitFromCurrentUowScope(ILogger logger)
        {
            ...
            if (unitOfWorkKey == null)
            {
                logger.Warning("There is no current UOW to exit!");
                return;
            }
            ...
            {
                //No outer UOW
                logger.Warning("Outer UOW key could not found in UnitOfWorkDictionary!");
                CallContext.FreeNamedDataSlot(ContextKey);
                return;
            }
